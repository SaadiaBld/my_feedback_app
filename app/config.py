...
# --- Forcer SSL sur Render (si nécessaire) ---
if os.getenv("RENDER", "false").lower() == "true" or os.getenv("RENDER_EXTERNAL_URL"):
    uri = Config.SQLALCHEMY_DATABASE_URI
    if uri and "sslmode=" not in uri and uri.startswith("postgresql://"):
        sep = "&" if "?" in uri else "?"
        Config.SQLALCHEMY_DATABASE_URI = uri + f"{sep}sslmode=require"

# --- Logging court, sans secrets (partout, utile en debug) ---
try:
